...
std::string perform_control_sequences(const std::string & target) {
    ...
    for (size_t input_pos = 0; input_pos < strlen(raw_string); input_pos++) {
        ...
        if (current_value == '\n') {
            ...
            current_column = 0;
        } else if (current_value == '\x1b' && state != OSC_STRING) {
            state = ESC;
            ...
            // its BEL ("\x07") terminator.
            state = OSC_STRING;
        } else if (state == OSC_STRING) {
            if (current_value == '\x07') {
                state = EMPTY;
            }
...
// Extracts all OSC 9;4 taskbar progress sequences ("\x1b]9;4;<state>;<percent>\x07")
...
std::vector<std::string> extract_taskbar_progress(const std::string & text) {
    ...
    std::regex osc_regex("\\x1b\\]9;4;(\\d+;\\d+)\\x07");
...
void ProgressbarInteractiveTest::test_perform_control_sequences() {
    ...
    // visible screen and are consumed up to their BEL terminator.
    expected = "aaabbb";
    CPPUNIT_ASSERT_EQUAL(
        expected,
        perform_control_sequences("aaa\x1b]9;4;1;42\x07"
                                  "bbb"));
    CPPUNIT_ASSERT_EQUAL(
        expected,
        perform_control_sequences("aaa\x1b]9;4;0;0\x07"
                                  "bbb"));
}
